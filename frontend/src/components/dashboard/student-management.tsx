    ...
    <div className="space-y-6 lg:space-y-8">
      ...
      >
        ...
        <div hidden={activeTab !== "students"}>
          <StudentsTab role={role} courseOptions={courseOptions} courseOptionsLoading={courseOptionsLoading} />
        </div>
...
function StudentsTab({ role, courseOptions, courseOptionsLoading }: StudentsTabProps) {
  ...
  const { showToast } = useToast();
  const canViewDetail = role === "TEACHER";
  ...
  }, [appliedKeyword, courseId, status, page, showToast]);
  ...
  useEffect(() => {
    if (!detailId || !canViewDetail) {
      return;
    }
    setDetail(null);
    setDetailError(null);
    setDetailLoading(true);
    fetchStudentCourseDetail(detailId)
      .then((response) => {
        setDetail(response);
      })
      .catch((err) => {
  ...
  }, [detailId, canViewDetail, showToast]);
  ...
  return (
    <div className="space-y-6">
      <Card title="학생 필터" description="Course, 상태, 검색어를 조합해 원하는 학생만 추려보세요.">
        ...
        {role === "ASSISTANT" && (
          <p className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-xs text-slate-500">
            조교는 학생 상세 정보(연락처·기록)를 열람할 수 없습니다. 목록 데이터만 참고해 주세요.
          </p>
        )}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>수강 반</TableHead>
                  <TableHead>상태</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {students.map((student, index) => {
                  const recordId = student.recordId ?? `record-${index}`;
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={() => {
                        if (!canViewDetail || !student.recordId) return;
                        setDetailId(student.recordId);
                      }}
                    >
                      <TableCell>
                        <div className="flex flex-col text-sm">
                          <span className="font-semibold text-slate-900">{student.studentName ?? "-"}</span>
                          <span className="text-xs text-slate-500">{student.studentMemberId}</span>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex flex-col text-sm">
                          ...
                          {student.assistantMemberId && (
                            <span className="text-xs text-slate-500">담당 조교: {student.assistantMemberId}</span>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
      ...
      <StudentDetailModal
        role={role}
        open={Boolean(detailId && canViewDetail)}
...
function StudentDetailModal({
  ...
  error: string | null;
  onClose: () => void;
  onDetailChange: (detail: StudentCourseDetailResponse) => void;
  onUpdated?: () => void;
}) {
  const { showToast } = useToast();
  ...
  const canEdit = role === "TEACHER" && Boolean(detail?.recordId);
  ...
  useEffect(() => {
    ...
    setAssistantInput(detail.assistantMemberId ?? "");
    setClinicInput(detail.defaultClinicSlotId ?? "");
    setNotesInput(detail.teacherNotes ?? "");
    setEditing(false);
  }, [detail]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await updateStudentCourseRecord(detail.recordId, payload);
        ...
        <div className="space-y-6">
          <section>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              <Badge variant={detail.active ? "success" : "secondary"}>{detail.active ? "활성" : "비활성"}</Badge>
              <span className="text-xs text-slate-500">{detail.recordId}</span>
            </div>
          </section>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <InfoCard
              title="Course 정보"
              items={[
                { label: "반 이름", value: detail.course?.name ?? "-" },
                { label: "지점", value: detail.course?.branchName ?? "-" },
                { label: "회사", value: detail.course?.companyName ?? "-" },
                { label: "기간", value: formatCoursePeriod(detail.course?.startDate, detail.course?.endDate) }
              ]}
            />
          </div>
            ...
            {!editing ? (
              <dl className="mt-3 space-y-2">
                <div className="flex justify-between gap-4">
                  <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">조교 ID</dt>
                  <dd className="text-right">{detail.assistantMemberId ?? "-"}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">기본 클리닉 슬롯</dt>
                  <dd className="text-right">{detail.defaultClinicSlotId ?? "-"}</dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">노트</dt>
                  <dd className="mt-1 whitespace-pre-wrap text-right">{detail.teacherNotes ?? "등록된 노트가 없습니다."}</dd>
                </div>
              </dl>
            ) : (
              <div className="mt-4 space-y-3">
                <Field label="조교 ID">
                  <Input
                    placeholder="조교 Member UUID"
                    value={assistantInput}
                    onChange={(event) => setAssistantInput(event.target.value)}
                    disabled={saving}
                  />
                </Field>
                <Field label="기본 클리닉 슬롯 ID">
                  <Input
                    placeholder="Clinic Slot UUID"
                    value={clinicInput}
