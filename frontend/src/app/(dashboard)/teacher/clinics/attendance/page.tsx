...
import { fetchCourseStudents, fetchTeacherBranchAssignments, fetchTeacherCourses } from "@/lib/dashboard-api";
...
import type { CourseResponse, CourseStudentResponse, TeacherBranchAssignment } from "@/types/dashboard";
...
export default function TeacherClinicAttendancePage() {
  ...
  const selectedCourseName = useMemo(
    () => courseOptions.find((course) => course.courseId === selectedCourseId)?.name ?? "",
  ...
  const {
    ...
    { dateRange: todayRange, branchId: selectedBranchId ?? undefined },
  ...
  const loadCourses = useCallback(async () => {
    ...
    try {
      const result = await fetchTeacherCourses({ status: "ACTIVE", branchId: selectedBranchId, page: 0, size: 100 });
  ...
  useEffect(() => {
    ...
    if (selectedCourseId && courseOptions.some((course) => course.courseId === selectedCourseId)) {
      return;
    }
    setSelectedCourseId(courseOptions[0]?.courseId ?? null);
  }, [courseOptions, isAddModalOpen, selectedCourseId]);
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      <Card title="오늘의 출석부" description="선택한 세션 기준으로 출석 명단을 관리합니다.">
        <div className="space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-500">오늘 세션을 조회할 지점을 선택하세요.</p>
          </div>
          ...
          {!branchLoading && branches.length === 0 && (
            <EmptyState message="연결된 지점이 없습니다." description="지점 연결 후 출석부를 관리할 수 있습니다." />
          )}
      ...
      <Card title="오늘 세션" description="오늘 진행되는 세션을 선택하세요.">
        <div className="space-y-4">
          ...
          {!sessionsLoading && !selectedBranchId && (
            <EmptyState message="지점을 먼저 선택해 주세요." description="지점 선택 후 오늘 세션이 표시됩니다." />
          )}

          {!sessionsLoading && selectedBranchId && sessions.length === 0 && (
            <EmptyState message="오늘 세션이 없습니다." description="오늘 일정이 없는 지점입니다." />
          )}
      ...
      <Modal open={isAddModalOpen} onClose={closeAddModal} title="출석 추가" size="md">
        <div className="space-y-4">
          {courseLoading && <Skeleton className="h-12" />}
