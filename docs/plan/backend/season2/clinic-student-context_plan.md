...
- 학생 화면에서 기본 슬롯/세션의 “정원 대비 현재 참석 수”를 보여줄 수 없어 선택 시 확신이 부족하다.
- 학생이 추가 참석을 신청한 뒤 취소할 수 있는 전용 API가 없다.
...
3. **주간 세션 참석 수 제공**
   - `ClinicSessionResponse`에 `attendanceCount`를 포함해 정원 대비 현재 참석 수를 제공한다.
   - 참석 수는 취소/삭제되지 않은 활성 출석만 집계한다.
4. **학생 참석 취소**
   - 학생이 본인 출석을 취소할 수 있는 API를 제공한다.
   - 세션 시작 30분 전 이후에는 취소가 불가하다(기존 잠금 규칙과 동일).
   - 기본 슬롯에서 자동 생성된 출석은 취소할 수 없다.
...
### 3.3 ClinicSession Response Extension
- `GET /api/v1/clinic-sessions?...`
  - Response: `ClinicSessionResponse`에 `attendanceCount` 추가
  - Count 기준: 취소되지 않은 활성 출석 수

### 3.4 Student Attendance Cancel
- `DELETE /api/v1/students/me/clinic-attendances/{attendanceId}`
  - Response: `RsData<ClinicAttendanceResponse>`
  - Errors: `CLINIC_ATTENDANCE_LOCKED`, `CLINIC_ATTENDANCE_NOT_FOUND`, `FORBIDDEN`
  - Note: 학생 본인 출석만 취소 가능
...
- 변경 DTO
  - `ClinicSessionResponse` (`attendanceCount` 추가)
...
5. `ClinicSessionQueryServiceTest`
   - 세션 응답에 `attendanceCount`가 포함되는지 검증
6. `ClinicAttendanceControllerTest` (student cancel)
   - `DELETE /api/v1/students/me/clinic-attendances/{attendanceId}` 성공/실패 시나리오 검증
