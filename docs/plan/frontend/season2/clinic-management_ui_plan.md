...
- `ClinicSlotPanel`
  ...
  - **어떻게 동작**:
    - 월~일 가로축, 06:00~22:00 세로축의 고정 시간표 그리드로 렌더한다.
    - 구현 시 `/teacher/courses`의 **캘린더 뷰** UI/구조/상호작용을 참고한다.
    - 날짜 이동은 불가하며, 고정된 주간 패턴만 표현한다.
    - 셀 드래그/롱프레스 → 슬롯 생성 모달을 호출한다.
    - 기존 슬롯은 블록으로 표시하며 Teacher는 수정/삭제, Assistant는 생성/조회 흐름만 제공한다.
...
- 범위:
  ...
  - Assistant: 슬롯 생성/조회 연결 (수정/삭제 미노출)
