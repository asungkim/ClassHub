...
| ClinicSlot                 | `courseId`, `teacherMemberId`, `creatorMemberId`, `branchId`, `dayOfWeek`, `startTime`, `endTime`, `defaultCapacity`, `deletedAt` |
| ClinicSession              | `slotId (nullable)`, `sessionType (REGULAR/EMERGENCY)`, `creatorMemberId (nullable)`, `date`, `capacity`, `isCanceled` |
...
| Invitation                 | `senderId (Teacher)`, `targetEmail`, `inviteeRole (ASSISTANT)`, `code`, `status`, `expiredAt`, `deletedAt`         |
...
- **Soft Delete**: Company, Branch, Member, TeacherBranchAssignment, TeacherAssistantAssignment, Course, StudentCourseRecord, ClinicSlot, Invitation은 `deletedAt` 타임스탬프로 삭제 관리. `deletedAt IS NULL` 조건으로 활성 데이터만 조회
- **Invitation**: ASSISTANT만 초대 가능, targetEmail 1:1, 사용 시 `deletedAt` 설정 및 TeacherAssistantAssignment 자동 생성
...
- **ClinicSlot 반별 관리**: courseId 필수, 반별 독립적인 클리닉 시간표, creatorMemberId로 생성자 추적
- **ClinicSession 타입**: REGULAR (slotId 기반 자동 생성), EMERGENCY (수동 생성, slotId=null, creatorMemberId 기록)
...
- sessionType = REGULAR, slotId 설정, capacity = ClinicSlot.defaultCapacity
...
| Method | URI                    | Description                                                                            | Role                                     |
| ------ | ---------------------- | -------------------------------------------------------------------------------------- | ---------------------------------------- |
| POST   | /clinic-slots          | Course/Branch/Teacher 기준 Slot 생성 (`dayOfWeek`, `startTime`, `defaultCapacity`), creatorMemberId 자동 기록 | TEACHER                                  |
...
- Slot은 Course별 독립 일정표이며, soft delete 시 해당 Slot 기반 Session 생성이 중지된다.
...
| POST   | /clinic-slots/{slotId}/sessions      | 정규 세션 수동 생성 (sessionType=REGULAR, slotId 설정, capacity=slot.defaultCapacity) | TEACHER            |
| POST   | /clinic-sessions/emergency           | 긴급 세션 생성 (sessionType=EMERGENCY, slotId=null, creatorMemberId 기록, capacity 직접 지정) | TEACHER, ASSISTANT |
...
- EMERGENCY Session은 Teacher/Assistant가 수동으로 생성하며, slotId 없이 독립적으로 운영된다.
...
| DELETE | /invitations/{id}            | 초대 취소 (soft delete, status=REVOKED)                              | TEACHER                              |
| POST   | /auth/verify-invitation      | code 검증                                                            | GUEST                                |
| POST   | /auth/register/by-invitation | 초대 기반 가입                                                       | GUEST                                |

- Invitation 상태: PENDING → ACCEPTED (사용 완료) / REVOKED (Teacher 취소) / EXPIRED (만료 시 배치로 처리)이며, `deletedAt IS NOT NULL`이면 더 이상 검증되지 않는다.
