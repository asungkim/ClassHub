...
- **Course 상태 계산**: 저장하지 않고 `startDate/endDate` 기준으로 계산한다.
  - `UPCOMING`: 현재 < `startDate`
  - `ACTIVE`: `startDate` ≤ 현재 ≤ `endDate`
  - `ENDED`: 현재 > `endDate`
  - Course 상태는 조회/표시 용도로만 사용하며 진도/기록 작성은 막지 않는다.
...
| StudentTeacherRequests    | /student-teacher-requests                            | 학생 요청 생성/조회/승인/거절/취소                                    |
...
| StudentCourses            | /student-courses, /students/me/courses               | Record 조회, teacherNotes/defaultClinicSlot/담당 조교 수정            |
...
- Course 응답에는 계산된 `status`(UPCOMING/ACTIVE/ENDED)를 포함한다.

### 4.5 StudentTeacherRequests

| Method | URI                                         | Description                                      | Role    |
| ------ | ------------------------------------------- | ------------------------------------------------ | ------- |
| POST   | /student-teacher-requests                   | 학생이 `teacherId`, `message`로 요청 생성         | STUDENT |
| GET    | /student-teacher-requests/me?status         | 학생 본인 요청 목록 조회                          | STUDENT |
| PATCH  | /student-teacher-requests/{id}/cancel       | 학생 요청 취소                                    | STUDENT |
| GET    | /student-teacher-requests?status&studentName | Teacher가 대기/처리 내역 조회                     | TEACHER |
| PATCH  | /student-teacher-requests/{id}/approve      | 승인 처리 → TeacherStudentAssignment 생성         | TEACHER |
| PATCH  | /student-teacher-requests/{id}/reject       | 거절 처리 (`message` optional)                    | TEACHER |
...
| Method | URI                                           | Description                                        | Role               |
| ------ | --------------------------------------------- | -------------------------------------------------- | ------------------ |
| GET    | /teacher-students?courseId&status&keyword     | 연결된 학생 목록 조회 (반 필터/상태 필터 포함)     | TEACHER, ASSISTANT |
| GET    | /teacher-students/{studentId}                | 학생 상세 조회 (배치된 반 목록/상태 포함)          | TEACHER, ASSISTANT |
| POST   | /student-course-assignments                   | 학생을 반에 배치 (`studentId`, `courseId`)          | TEACHER, ASSISTANT |
| POST   | /student-course-assignments/bulk              | 학생을 반에 일괄 배치 (`courseId`, `studentIds`)     | TEACHER, ASSISTANT |
| PATCH  | /student-course-assignments/{id}/off          | 휴원 처리 (`deletedAt` 설정)                        | TEACHER, ASSISTANT |
| PATCH  | /student-course-assignments/{id}/on           | 재원 처리 (`deletedAt` 복구)                        | TEACHER, ASSISTANT |
...
- `status`는 `ACTIVE`, `INACTIVE`, `UNASSIGNED`, `ALL` 중 하나이며 기본은 ACTIVE다.
  - `UNASSIGNED`: TeacherStudentAssignment는 있으나 StudentCourseAssignment가 전혀 없음 (배치 전)
  - `ACTIVE`: StudentCourseAssignment가 1개 이상 활성 (재원)
  - `INACTIVE`: StudentCourseAssignment가 존재하지만 전부 비활성 (휴원)
  - `ALL`: TeacherStudentAssignment가 있는 모든 학생
- `courseId`가 있으면 해당 반 기준으로 필터링한다.
- 휴원/재원 처리는 학생 상세 화면에서 StudentCourseAssignment 단위로 수행한다.
