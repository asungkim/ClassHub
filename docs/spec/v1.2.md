...
- 본 스펙은 `docs/requirement/v1.2.md`를 기준으로 작성하며, 요구사항 변경 시 먼저 Requirement 문서를 업데이트한 뒤 스펙/ TODO 순서로 버전을 올린다.
- Functional Requirement 추적 표

| FR ID | 요구사항 요약 (`docs/requirement/v1.2.md`) | 스펙 매핑 (Section / Resource) |
| --- | --- | --- |
| FR-001 | 이메일/비밀번호 + 소셜 기반 로그인/회원가입 | 3.2 Auth API (로그인/회원가입/초대) |
| FR-002 | Teacher 대시보드 조회 | 3.2 Courses/Students + 5. Phase 5 Dashboard Epic |
| FR-003 | 대시보드 위젯 커스터마이징 | 5. Phase 5 Dashboard Epic (위젯 저장/로드) |
| FR-004 | SharedLesson 생성 | 3.2 SharedLesson POST |
| FR-005 | SharedLesson 수정/삭제 | 3.2 SharedLesson PATCH/DELETE |
| FR-006 | 날짜별 SharedLesson 조회 | 3.2 SharedLesson GET (필터) |
| FR-007 | PersonalLesson 생성 | 3.2 PersonalLesson POST |
| FR-008 | PersonalLesson 수정/삭제 | 3.2 PersonalLesson PATCH/DELETE |
| FR-009 | 날짜별 PersonalLesson 조회 | 3.2 PersonalLesson GET (필터) |
| FR-010 | 학생별 클리닉 조회 | 3.2 ClinicRecord/ClinicSession + StudentProfile timeline |
| FR-011 | ClinicRecord 작성 | 3.2 ClinicRecord POST |
| FR-012 | 조교 공지사항 CRUD | 3.2 Notice 리소스 |
| FR-013 | 공지 읽음 체크 | 3.2 NoticeReads |
| FR-014 | 조교 초대 | 3.2 Invitations + Auth register/by-invitation |
| FR-015 | 조교 퇴직 처리 | 3.2 Members deactivate + CourseAssistant API |
| FR-016 | 조교 근무시간 조회 | 3.2 WorkLogs (집계) |
| FR-017 | 조교 근무시간 CRUD | 3.2 WorkLogs |
| FR-018 | 반별 학생 목록 조회 | 3.2 Courses/{id}/students, StudentProfiles |
| FR-019 | 학생 캘린더(공통/개별/클리닉) | 3.2 StudentProfiles timeline API |
| FR-022 | 학생 CRUD | 3.2 StudentProfiles |
| FR-023 | Course CRUD | 3.2 Courses |
| FR-024 | Assistant가 전체 수업 내용 확인 | 3.2 Shared/Personal/Clinic 조회 권한 |
| FR-025 | Admin 전체 권한 | 3.2 Members API + Admin 제어 Epic |
| FR-026 | Assistant가 Student 초대 | 3.2 Invitations (inviteeRole=STUDENT) |
| FR-027a | Student ClinicSession 변경 | 3.2 ClinicAttendance/ClinicSession 변경 API |
| FR-027b | ClinicSlot CRUD | 3.2 ClinicSlots / 세션 생성 |
