...
- ⚪ 선생님 회원가입 검증 및 리팩터링 (기존 로직 정리)
- ⚪ 학생 회원가입 신규 구현
- ⚪ 로그인/로그아웃 플로우 회귀 테스트
- ⚪ 조교 초대 + verify API 검증 및 리팩터링
- ⚪ 조교 회원가입 신규 구현
  - ⚪ TeacherAssistantAssignment 자동 생성
  - ⚪ 초대 수락 시 Assignment 자동 연결
- ⚪ 토큰 재발급(`/auth/refresh`) 검증 및 리팩터링
- ⚪ 로그인 사용자 정보(`/auth/me`) 응답 구조 검증 및 리팩터링
...
- ⚪ Repository TDD 구현 (`backend/AGENTS.md` 기준)
  - 각 Repository에 필요한 최소한의 쿼리 만들기
  - 각 엔티티별 JpaRepository + 커스텀 쿼리 메서드
  - 단위 테스트 (쿼리 메서드, 연관관계, 제약조건 검증)
