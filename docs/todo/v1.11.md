# TODO v1.11 (Requirement v1.5 → Spec v1.5 기준)

> 요구사항 출처: `docs/requirement/v1.5.md` (Use Case/흐름)
> 스펙 반영: `docs/spec/v1.5.md`
...
> 참고 문서 : `docs/design/**` , `docs/requirement/v1.5` , `docs/spec/v1.5`
...
> 목표: 핵심 워크플로(Course/학생연결·배치/Lesson/Clinic/Invitation) CRUD 우선 구현. Notice/WorkLog는 Phase 7에서 추가 기능으로 개발.
...
- ⚪ Course 상태 계산 필드 추가 (UPCOMING/ACTIVE/ENDED)
...
- ✅ Course 관리 페이지 (Teacher)
  ...
  - Course 시간표 페이지
- ✅ Course 페이지 (SuperAdmin, Assistant, Student)
- ⚪ Course 상태 배지 표시 (진행전/진행중/종료)

---

## 🔹 Epic: Enrollment 워크플로 (v1.4 완료)

> v1.5부터는 학생-선생님 연결/반 배치 흐름으로 전환하며, 관련 작업은 아래 Epic에서 진행한다.

### 백엔드

> 참고 문서 : student-enrollment-management_plan.md

- ✅ Student 기능 API 개발
- ✅ Teacher/Assistant 기능 API 개발
- ✅ Admin 기능 API 개발
...
- ✅ 내 수업 페이지 (/student/my-courses)
  - ✅ 수업 목록 페이지 api 연동
  - ✅ 신청 내역 페이지
    - ✅ 내역 조회 api 연동
    - ✅ 요청 취소 api 연동
- ✅ 반 검색 페이지 (/student/course/search)
  - ✅ api 연동 (StudentCourseEnrollment 생성 + 내 수업 신청 내역으로 이동)
...
> 참고 문서 : progress-management_plan.md

### 백엔드

- ✅ CourseProgress CRUD
- ✅ PersonalProgress CRUD
...
- ✅ 통합 수업 작성 모달(Teacher)
  - 반 선택 후 CourseProgress + 쓰고 싶은 학생 선택 후 PersonalProgress 작성
- ✅ CourseProgress 관리 페이지 (Teacher)
- ✅ PersonalProgress 관리 페이지 (Teacher)
...
- ✅ ClinicSession 개발
  - ✅ CRUD 개발
  - ✅ 세션 응답에 attendanceCount 제공
- ✅ ClinicAttendance API
  - ✅ CRUD 개발
  - ✅ 학생 참석 취소 API 추가
...
- ✅ ClinicSlot 페이지
  - Teacher 페이지 (지점별 클리닉)
  - Assistant 페이지 (선생님별 클리닉)
    - teacher & branch 로 묶기
  - Student 페이지 (클리닉 시간표)
- ✅ ClinicSession 페이지
  - Teacher 페이지 (주차별 클리닉)
  - Assistant 페이지 (주차별 클리닉)
  - Student 페이지 (선생님별 이번주 클리닉)
    - teacher & branch 로 묶기
- ✅ ClinicAttendace 페이지
  - Teacher,Assistant 페이지 (오늘의 출석부)
  - Student 목록 페이지 (이번주 클리닉)
- ✅ ClinicRecord CRUD (Teacher, Assistant)

---

## 🔹 Epic: 학생-선생님 연결 & 반 배치 (v1.5 전환)

### 백엔드

- ⚪ Teacher 검색 API (이름 + Company/Branch 필터, VERIFIED만)
- ⚪ StudentTeacherRequest 생성/조회/취소 API (학생)
- ⚪ StudentTeacherRequest 승인/거절 API + TeacherStudentAssignment 생성 (Teacher)
- ⚪ TeacherStudentAssignment 기반 학생 목록 조회 (상태 필터: 배치 전/재원/휴원/전체)
- ⚪ StudentCourseAssignment 배치/휴원/재원 API (Teacher/Assistant)
- ⚪ Course 폐강 시 StudentCourseAssignment 비활성화 처리
- ⚪ 기존 Enrollment 흐름 제거 및 권한/쿼리 전환 (StudentCourseAssignment 기준)
- ⚪ 클리닉/진도/캘린더 권한을 StudentCourseAssignment 기준으로 전환

### 프론트엔드

- ⚪ 학생 선생님 검색/요청 페이지
- ⚪ 학생 요청 상태/연결 목록 페이지
- ⚪ Teacher 요청 관리 페이지 (승인/거절)
- ⚪ Teacher 학생 목록 + 상태 필터 + 반 배치 UI
- ⚪ Teacher/Assistant 반 배치 on/off UX

---

## 🔹 Epic: 기존 Enrollment 코드 정리

- ⚪ StudentEnrollmentRequest/StudentCourseEnrollment 관련 엔티티/레포/서비스/컨트롤러/테스트 제거
- ⚪ 기존 Enrollment API 제거 및 문서/테스트 정리
...
- ⚪ Teacher 플로우: Company 등록 → Branch 생성 → Course 생성 → 학생 연결 요청 승인 → 반 배치 → Lesson 작성 → 클리닉 운영
- ⚪ Assistant 플로우: 초대 코드 수락 → Assignment 확인 → 학생 배치/휴원 처리 → Clinic 일정 확인 → Attendance 처리 → Record 작성
- ⚪ Student 플로우: Teacher 검색 → 연결 요청 → 승인 대기 → 반 배정 확인 → 캘린더/클리닉 확인 → Attendance 변경
