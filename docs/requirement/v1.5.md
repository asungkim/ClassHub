...
- **반 배치/해제**: 반 생성 후 연결된 학생을 배치한다.
  ...
  - 휴원/재원은 StudentCourseAssignment on/off로 관리
    - 휴원 시 StudentCourseRecord 비활성화(soft delete) 처리
    - 휴원 시점 이후의 ClinicAttendance 삭제 (해당 반 기준)
    - 휴원 상태에서는 기본 슬롯 설정/클리닉 참석 신청/이동 불가
    - 재원 시 StudentCourseRecord 복구 및 이번 주 기본 슬롯 출석 재생성
- **학생 기록 관리**: StudentCourseRecord의 `teacherNotes`, 담당 조교, 기본 클리닉 슬롯을 관리한다.

#### 수업 관리

- **Course CRUD**: 반 생성, 조회, 수정, 활성화, 비활성화 (branchId 필수)
  - 반 종료일 기준 7일 뒤 자동 보관 모드 전환 (Course soft delete)
  - 종료일부터 보관 전까지 경고 표시 (프론트)
...
- **ClinicAttendance 관리**: 세션별 학생 출석 조회
  ...
  - 보관된 반(Course deletedAt)에는 자동 배정 중단
  - 긴급 세션은 수동으로 학생 추가
