...
- **StudentCourseRecord 관리**: 학생의 defaultClinicSlotId 설정/변경
  - 학생 요청 시 고정 클리닉 슬롯 변경 (동일 슬롯 중복 선택/시간 겹침 불가)
...
- **ClinicSlot 조회 및 선택**: 본인이 수강하는 반의 선생님/지점 기준 클리닉 시간표 확인
...
- **내 참석 세션 목록 조회**: 이번 주 기준 본인 ClinicSession/Attendance 목록 확인
  - 추가 신청/이동은 동일 teacher+branch 세션 목록에서 선택
...
| **ClinicSlot**                 | 클리닉 시간표 (teacher/branch 기준, 요일별 반복, defaultCapacity, creatorMemberId, Soft Delete)      |
| **ClinicSession**              | 특정 날짜의 실제 클리닉 세션 (sessionType: REGULAR/EMERGENCY, startTime/endTime, capacity, isCanceled) |
...
| Course ↔ ClinicSlot (논리 연결)              | 동일 teacher/branch 슬롯 공유          |
...
- **ClinicSlot**: teacher/branch 기준 시간표로 명확화 (Course와 직접 FK 없음)
- **ClinicSession**: startTime/endTime 필드 추가 (긴급 세션 시간 입력)
- **StudentCourseRecord**: defaultClinicSlotId 검증 규칙 강화 (teacher/branch 일치, 중복/시간 겹침 금지)

### 사용자 흐름 변경

- 변경 없음 (슬롯 공유 규칙/검증 기준만 갱신)

### Use Case 변경

- 변경 없음 (긴급 세션 생성 시 시간 입력만 명시)

### 데이터 모델 개선

- Course ↔ ClinicSlot은 teacherMemberId + branchId 논리 관계로 연결

### 클리닉 프로세스 상세화

1. **ClinicSlot 생성** (Teacher):

   - 선생님/지점 기준으로 슬롯 생성 (요일, 시간, 정원)
   - 동일 teacher + branch의 Course 학생들은 동일 슬롯 목록을 공유
   - 슬롯 생성 시 이번 주 남은 요일에 대해 ClinicSession을 생성 (해당 요일 시간이 지나지 않았을 때)

2. **고정 클리닉 선택** (Student):

   - 반 등록 승인 후 Course별 고정 클리닉 슬롯 선택 (defaultClinicSlotId)
   - 동일 슬롯 중복 선택/시간 겹침은 허용하지 않음
   - defaultClinicSlotId 설정 인원은 slot.defaultCapacity를 초과할 수 없음
   - 선택 후 학생이 직접 변경 불가 (악용 방지)
   - 변경 필요 시 선생님에게 요청
   - 변경은 다음 주부터 적용 (이번 주 변경은 동일 주차 이동으로 처리)

3. **주차별 세션 자동 생성** (REGULAR 타입):

   - 매주 일요일 자정에 해당 주차(월~일) ClinicSession 자동 생성
   - sessionType = REGULAR, slotId 자동 설정
   - ClinicSlot의 시간(startTime/endTime)과 defaultCapacity를 Session에 복사
...
4. **긴급 세션 수동 생성** (EMERGENCY 타입):
   ...
   - 날짜, startTime/endTime, 정원을 직접 설정
