...
- **전체 권한 보유**
- **Company 검증 관리**:
  - 선생님이 등록한 Company를 VERIFIED 처리 (UNVERIFIED → VERIFIED)
  - Company는 UNVERIFIED, VERIFIED 상태만 존재
- **Feedback 관리**:
...
1. **Teacher가 생성한 초대 코드로 회원가입**:
   - Invitation code 입력 → ASSISTANT role 자동 부여 (단일 역할)
   - Invitation은 targetEmail과 1:1 매핑
2. **TeacherAssistantAssignment 자동 생성**
   - Invitation 사용 시 isActive = false 처리
...
| 엔티티                         | 설명                                                                     |
| ------------------------------ | ------------------------------------------------------------------------ |
| **Company**                    | 학원/회사 정보 (type: INDIVIDUAL/ACADEMY, status: UNVERIFIED/VERIFIED)   |
| **Branch**                     | 학원 지점 정보                                                           |
| **Member**                     | 사용자 엔티티 (단일 역할: Teacher/Assistant/Student/Admin/SuperAdmin)    |
| **StudentInfo**                | 학생 추가 정보 (학교, 학년, 생년월일, 학부모 연락처)                     |
| **TeacherBranchAssignment**    | 선생님-지점 배정 (M:N 관계, role: OWNER/FREELANCE)                       |
| **TeacherAssistantAssignment** | 선생님-조교 배정 (M:N 관계)                                              |
| **Course**                     | 반 정보 (Teacher가 특정 Branch에 생성, schedules: ElementCollection)     |
...
| **SharedLesson**               | 반 공통 진도 (Course 삭제 시 CASCADE)                                    |
| **PersonalLesson**             | 학생별 개별 진도 (실제 DELETE 허용)                                      |
| **ClinicSlot**                 | 클리닉 시간표 (요일별 반복, capacity)                                    |
| **ClinicSession**              | 특정 날짜의 실제 클리닉 세션 (isCanceled)                                |
...
| **Invitation**                 | 조교 초대 코드 (ASSISTANT만, isActive, PENDING/ACCEPTED/EXPIRED/REVOKED) |
...
- **Company, Branch**: 멀티 학원/지점 구조 지원 (Company는 UNVERIFIED/VERIFIED만)
- **StudentInfo**: Member와 역할별 정보 분리 (TeacherInfo 제거)
...
- **Teacher**: Company/Branch 관리, StudentEnrollmentRequest 승인 추가 (role: OWNER/FREELANCE)
- **Assistant**: 학생 초대 제거, Invitation 방식 유지 (targetEmail 1:1)
...
- **Invitation**: branchId 제거, isActive 추가, targetEmail과 1:1 매핑
- **SharedLesson**: Course 삭제 시 CASCADE 적용
